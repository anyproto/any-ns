...
const AnytypeRegistrar = artifacts.require(
  './anytype/AnytypeRegistrarImplementation',
)
...
contract('AnytypeRegistrar', function (accounts) {
  ...
  before(async () => {
    ...
    registrar = await AnytypeRegistrar.new(ens.address, namehash.hash('any'), {
      from: ownerAccount,
    })
    await registrar.addController(controllerAccount, { from: ownerAccount })
    await ens.setSubnodeOwner('0x0', sha3('any'), registrar.address)
  })
  ...
  it('should allow new registrations', async () => {
    ...
    assert.equal(
      await ens.owner(namehash.hash('newname.any')),
      registrantAccount,
    )
  ...
  it('should allow registrations without updating the registry', async () => {
    ...
    var block = await web3.eth.getBlock(tx.receipt.blockHash)
    assert.equal(await ens.owner(namehash.hash('silentname.any')), ZERO_ADDRESS)
  ...
  it('should permit the owner to reclaim a name', async () => {
    ...
    await ens.setSubnodeOwner(ZERO_HASH, sha3('any'), registrar.address)
  ...
  it('should allow the owner to set a resolver address', async () => {
    ...
    assert.equal(await ens.resolver(namehash.hash('any')), accounts[1])
  })
})
