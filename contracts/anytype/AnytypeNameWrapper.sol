...
import {ERC1155Fuse, IERC165, IERC1155MetadataURI} from "../wrapper/ERC1155Fuse.sol";
import {Controllable} from "../wrapper/Controllable.sol";
import {INameWrapper, CANNOT_UNWRAP, CANNOT_BURN_FUSES, CANNOT_TRANSFER, CANNOT_SET_RESOLVER, CANNOT_SET_TTL, CANNOT_CREATE_SUBDOMAIN, CANNOT_APPROVE, PARENT_CANNOT_CONTROL, CAN_DO_EVERYTHING, IS_DOT_ETH, CAN_EXTEND_EXPIRY, PARENT_CONTROLLED_FUSES, USER_SETTABLE_FUSES} from "../wrapper/INameWrapper.sol";
import {INameWrapperUpgrade} from "../wrapper/INameWrapperUpgrade.sol";
...
contract AnytypeNameWrapper is
    Ownable,
    ERC1155Fuse,
    INameWrapper,
    Controllable,
    ...
    ReverseClaimer
{
    ...
    string public constant name = "AnytypeNameWrapper";
     ...
     * @param id Label as a string of the .any domain to wrap
    ...
    /**
     * @notice Wraps an .any domain, creating a new token and sending the original ERC721 token to this contract
     * @dev Can be called by the owner of the name on the .any registrar or an authorised caller on the registrar
     * @param label Label as a string of the .any domain to wrap
    ...
    function wrapETH2LD(
        ...
        if (
            ...
            revert Unauthorised(
                _makeNode(ANY_NODE, bytes32(tokenId)),
                msg.sender
            );
        }
    ...
    /**
     * @dev Registers a new .any second-level domain and wraps it.
     *      Only callable by authorised controllers.
     * @param label The label to register (Eg, 'foo' for 'foo.any').
     * @param wrappedOwner The owner of the wrapped name.
     * @param duration The duration, in seconds, to register the name for.
     * @param resolver The resolver address to set on the ENS registry (optional).
     * @param ownerControlledFuses Initial owner-controlled fuses to set
     * @return registrarExpiry The expiry date of the new name on the .any registrar, in seconds since the Unix epoch.
     */
    ...
    /**
     * @notice Renews a .any second-level domain.
     * @dev Only callable by authorised controllers.
     * @param tokenId The hash of the label to register (eg, `keccak256('foo')`, for 'foo.any').
     * @param duration The number of seconds to renew the name for.
     * @return expires The expiry date of the name on the .any registrar, in seconds since the Unix epoch.
     */
    ...
    function renew(
        ...
        bytes32 node = _makeNode(ANY_NODE, bytes32(tokenId));
    ...
    /**
     * @notice Wraps a non .any domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
    ...
    /**
     * @notice Unwraps a .any domain. e.g. vitalik.any
     * @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
     * @param labelhash Labelhash of the .any domain
     * @param registrant Sets the owner in the .any registrar to this address
    ...
    function unwrapETH2LD(
        ...
        address registrant,
        address controller
    ) public onlyTokenOwner(_makeNode(ANY_NODE, labelhash)) {
        if (registrant == address(this)) {
            revert IncorrectTargetOwner(registrant);
        }
        _unwrap(_makeNode(ANY_NODE, labelhash), controller);
    ...
    /**
     * @notice Unwraps a non .any domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
     * @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
    ...
    /**
     * @notice Upgrades a domain of any kind. Could be a .any name vitalik.any, a DNSSEC name vitalik.xyz, or a subdomain
    ...
    function onERC721Received(
        ...
        //check if it's the any registrar ERC721
    ...
    function _beforeTransfer(
        ...
        // For this check, treat .any 2LDs as expiring at the start of the grace period.
        if (fuses & IS_DOT_ETH == IS_DOT_ETH) {
            expiry -= GRACE_PERIOD;
        }
    ...
    function _normaliseExpiry(
        ...
        // .any names will check registrar, non .any check parent
        if (expiry > maxExpiry) {
            expiry = maxExpiry;
        }
    ...
    function _wrapETH2LD(
        ...
        bytes32 labelhash = keccak256(bytes(label));
        bytes32 node = _makeNode(ANY_NODE, labelhash);
        // hardcode dns-encoded any string for gas savings
        bytes memory name = _addLabel(label, "\x03any\x00");
        names[node] = name;
